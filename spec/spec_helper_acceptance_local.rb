...
def local_splunk_host
  begin
    ...
    all_nodes = inventory_hash['groups'].detect { |g| g['name'] == 'ssh_nodes' }['targets']
    current_node = all_nodes.detect { |t| t['uri'] == ENV['TARGET_HOST'] }
...
def setup_manifest(disabled: false, cert_store: false, ssl_ca: nil, url: nil, with_event_forwarding: false)
  if url.nil?
    if ENV['SPLUNK_CI_URL'] && !ENV['SPLUNK_CI_URL'].empty?
      ...
      url = "https://#{local_splunk_host}:8088/services/collector"
    end
  end
